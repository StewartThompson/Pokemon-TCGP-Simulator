...
struct SimulateArgs {
    ...
    games: usize,
    #[arg(long, default_value = "grass", help = "Deck for player 1 (grass|fire)")]
    deck1: String,
    #[arg(long, default_value = "fire", help = "Deck for player 2 (grass|fire)")]
    deck2: String,
...
#[derive(Args)]
struct PlayArgs {
    #[arg(long, default_value = "grass", help = "Your deck (grass|fire)")]
    deck: String,
    #[arg(long, default_value = "fire", help = "Opponent's deck (grass|fire)")]
    opponent: String,
...
fn resolve_deck(db: &CardDb, name: &str) -> Result<(Vec<u16>, Vec<Element>), String> {
    let (ids, energy) = get_sample_deck(name)
        .ok_or_else(|| format!("Unknown deck '{}'. Available: grass, fire", name))?;
