...
// -------------------------------------------------------------------------- //
// Effect placeholder (replaced in Wave 5 when effects/mod.rs is integrated)
// -------------------------------------------------------------------------- //
...
fn parse_handler_simple(handler: &str) -> Vec<EffectKind> {
    // Simple parser: split on " | " and parse each as name(params)
    // Returns EffectKind placeholder structs
    if handler.is_empty() {
        return vec![];
    }
    handler.split(" | ").filter_map(|part| {
        let part = part.trim();
        ...
        if let Some(p) = paren {
            let args_str = &part[p+1..].trim_end_matches(')');
            for token in args_str.split(',') {
                ...
                if let Some(eq) = token.find('=') {
                    ...
                    if let Ok(n) = v.parse::<i64>() {
                        params.insert(k, ParamValue::Int(n));
                    } else if v.starts_with('(') && v.ends_with(')') {
                        let inner: Vec<String> = v[1..v.len()-1]
                            .split(',')
                            .map(|s| s.trim().to_string())
                            .collect();
                        params.insert(k, ParamValue::Tuple(inner));
                    } else {
                        params.insert(k, ParamValue::Str(v));
                    }
                } else if !token.is_empty() {
                    if let Ok(n) = token.parse::<i64>() {
                        params.insert("amount".to_string(), ParamValue::Int(n));
                    } else {
                        params.insert("amount".to_string(), ParamValue::Str(token.to_string()));
                    }
                }
            }
        }
        Some(EffectKind { name, params })
    }).collect()
}
...
fn parse_card(raw: RawCard) -> Card {
    ...
    let (kind, stage, element, weakness, hp, retreat_cost, attacks, ability,
        ...
        for ra in raw.attacks.unwrap_or_default() {
            ...
            let effects = parse_handler_simple(&handler);
            attacks.push(Attack {
                name: atk_name,
                damage,
        ...
        let trainer_handler = raw.abilities.as_ref()
            ...
            .unwrap_or_default();
        let trainer_effects = parse_handler_simple(&trainer_handler);
...
mod tests {
    ...
    fn assets_dir() -> PathBuf {
        ...
        d.pop(); // go up from ptcgp_rust/ to project root
        d.push("assets/cards");
        d
    }
